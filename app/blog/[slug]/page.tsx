...
export async function generateMetadata({
  ...
  const [post, lang] = await Promise.all([getBlogPost(slug), getServerLang()]);
    ...
    <div className="mx-auto w-full max-w-4xl px-3 sm:px-5 py-12">
      <div className="mb-8">
        {post.category_name ? (
          ...
          <Link
            href="/blog/"
            className="text-xs uppercase tracking-[0.2em] text-foreground/60 hover:text-foreground/80 transition-colors"
          >
            Blog
          </Link>
        )}
