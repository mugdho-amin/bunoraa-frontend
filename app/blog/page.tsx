...
export const metadata: Metadata = buildPageMetadata({
  ...
  description:
    "Explore Bunoraa's embroidery blog — care guides, stitching techniques, artisan stories, and Bangladeshi craft inspiration.",
  path: "/blog/",
});

async function getBlogPosts() {
  try {
    const response = await apiFetch<BlogPostListItem[]>("/pages/blog/");
    ...
    <div className="mx-auto w-full max-w-6xl px-3 sm:px-5 py-12">
      <div className="mb-8">
        ...
        <p className="mt-2 text-foreground/70">
          Care guides, stitching techniques, artisan stories, and Bangladeshi craft inspiration.
        </p>
      </div>
          ...
          <Link key={post.id} href={`/blog/${post.slug}/`} className="group block">
            <Card variant="bordered" className="flex flex-col gap-4">
              ...
              <div className="flex flex-1 flex-col gap-2 px-1 pb-1">
                ...
                {post.author_name ? (
                  <p className="text-xs text-foreground/50 mt-auto">By {post.author_name}</p>
                ) : null}
              </div>
            </Card>
          </Link>
