  ...
  if (!collections.length) {
    return (
      <div className="mx-auto w-full max-w-6xl px-[var(--page-gutter)] py-12 text-center">
        <div className="mb-8"><p className="text-sm uppercase tracking-[0.2em] text-muted-foreground">Collections</p><h1 className="text-3xl font-semibold">Curated sets</h1></div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {collections.map((collection) => (
          <Card key={collection.id} variant="bordered" className="flex flex-col gap-4">
            ...
            <div className="flex flex-1 flex-col gap-2"><h2 className="text-lg font-semibold">{collection.name}</h2><p className="text-sm text-muted-foreground">{collection.description}</p></div>
