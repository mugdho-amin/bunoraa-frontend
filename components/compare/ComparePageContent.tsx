      ...
      <div className="min-h-screen bg-background text-foreground">
        <div className="mx-auto w-full max-w-4xl px-[var(--page-gutter)] py-12">
          <Card variant="bordered" className="space-y-4 text-center">
            ...
            <p className="text-sm text-muted-foreground">Add products to compare from the catalog.</p>
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="mx-auto w-full max-w-6xl px-[var(--page-gutter)] py-12">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div><p className="text-sm uppercase tracking-[0.2em] text-muted-foreground">Compare</p><h1 className="text-3xl font-semibold">Side-by-side details</h1></div>
            ...
            {items.map((item) => {
              ...
              return (
                <Card key={item.id} variant="bordered" className="space-y-3 p-4">
                  ...
                  <div>{detail ? <ProductPrice price={detail.current_price} salePrice={detail.sale_price} currency={detail.currency || "BDT"} /> : <span className="text-sm text-muted-foreground">Loading...</span>}</div>
                  <div>{detail ? <RatingStars rating={detail.average_rating || 0} count={detail.reviews_count} size="sm" /> : <span className="text-sm text-muted-foreground">-</span>}</div>
                  ...
                  <div className="text-sm text-muted-foreground">{detail?.primary_category?.name || item.primary_category_name || "General"}</div>
                  <div className="text-xs text-muted-foreground line-clamp-3">{detail?.description || "No description"}</div>
