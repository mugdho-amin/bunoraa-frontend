...
export async function ArtisanDetailPageContent({ slug }: { slug: string }) {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-3 sm:px-5 py-12">
      <div className="mb-8">
        <p className="text-sm uppercase tracking-[0.2em] text-foreground/60">Artisan</p>
        <h1 className="text-3xl font-semibold">{artisan?.name || "Artisan profile"}</h1>
        <p className="mt-2 text-foreground/70">{artisan?.bio || "Artisan details are not available via API yet."}</p>
      </div>
      {products.length > 0 ? (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {products.map((product) => (
            <Card key={product.id} variant="bordered" className="flex flex-col gap-4">
              ...
              <div className="flex flex-1 flex-col gap-2"><h2 className="text-lg font-semibold">{product.name}</h2><p className="text-sm text-foreground/70">{product.short_description}</p></div>
              <Button asChild size="sm" variant="secondary"><Link href={buildProductPath(product)}>View product</Link></Button>
            </Card>
          ))}
        </div>
      ) : <Card variant="bordered" className="p-6 text-sm text-foreground/70">Products for this artisan are not available via the API yet.</Card>}
