...
export async function BundlesPageContent() {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-3 sm:px-5 py-12">
      <div className="mb-8"><p className="text-sm uppercase tracking-[0.2em] text-foreground/60">Bundles</p><h1 className="text-3xl font-semibold">Bundle deals</h1></div>
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {bundles.map((bundle) => (
          <Card key={bundle.id} variant="bordered" className="flex flex-col gap-4">
            ...
            <div className="flex flex-1 flex-col gap-2"><h2 className="text-lg font-semibold">{bundle.name}</h2><p className="text-sm text-foreground/70">{bundle.description}</p></div>
