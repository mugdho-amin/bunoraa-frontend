...
const schema = z.object({ new_password: z.string().min(8, "Password must be at least 8 characters"), new_password_confirm: z.string().min(8, "Confirm your new password") }).refine((data) => data.new_password === data.new_password_confirm, { message: "Passwords do not match", path: ["new_password_confirm"] });
...
export function ResetPasswordPageContent({ token }: { token: string }) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try { await apiFetch("/accounts/password/reset/", { method: "POST", body: { token, ...values } }); setComplete(true); setTimeout(() => router.push("/account/login/"), 1500); }
    catch { setErrorMessage("Unable to reset password. The link may have expired."); }
  };
