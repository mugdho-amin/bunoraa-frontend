...
export async function CategoriesPageContent() {
  ...
  if (!categories?.length) {
    return <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-6"><h1 className="text-2xl font-bold text-foreground/40 uppercase tracking-widest">{t("common.search.no_results")}</h1><Link href="/" className="mt-4 text-primary underline underline-offset-4 font-bold uppercase text-xs tracking-widest">{t("common.error.go_home")}</Link></div>;
  }
    ...
    <div className="min-h-screen bg-background text-foreground selection:bg-primary selection:text-white">
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 py-16 lg:py-24">
        <div className="mb-12 text-center sm:text-left animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <p className="mt-4 text-lg text-foreground/50 max-w-2xl leading-relaxed">Discover our meticulously crafted artisans products, organized by style and purpose.</p>
        </div>
