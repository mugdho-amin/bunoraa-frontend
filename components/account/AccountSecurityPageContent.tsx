...
import { decodeCreationOptions, encodeCredential } from "@/lib/webauthn";

const passwordSchema = z.object({ current_password: z.string().min(1, "Current password is required"), new_password: z.string().min(8, "Password must be at least 8 characters"), new_password_confirm: z.string().min(8, "Confirm your new password") }).refine((data) => data.new_password === data.new_password_confirm, { message: "Passwords do not match", path: ["new_password_confirm"] });
type PasswordForm = z.infer<typeof passwordSchema>;

function buildBackupCodesText(codes: string[]) {
  const issuedOn = new Date().toISOString();
  return ["Bunoraa Backup Codes", `Issued: ${issuedOn}`, "", "Each code can be used once. Keep these codes in a safe place.", "", ...codes, ""].join("\n");
}

export function AccountSecurityPageContent() {
  const form = useForm<PasswordForm>({ resolver: zodResolver(passwordSchema) });
  const { statusQuery, setupTotp, verifyTotp, disableTotp, regenerateBackupCodes, registerPasskeyOptions, registerPasskeyVerify, passkeysQuery, removePasskey } = useMfa();
  const { sessionsQuery, revokeSession, revokeOthers } = useSessions();

  const [totpFlow, setTotpFlow] = React.useState<"idle" | "setup" | "disable">("idle");
  ...
  const passkeyPending = registerPasskeyOptions.isPending || registerPasskeyVerify.isPending;

  const handlePasswordChange = async (values: PasswordForm) => { await apiFetch("/accounts/password/change/", { method: "POST", body: values }); form.reset(); };
  const handleTotpSetup = async () => { const response = await setupTotp.mutateAsync(); setTotpSecret(response.secret); setTotpUri(response.otpauth_url); setTotpSetupCode(""); setTotpFlow("setup"); setMfaNotice(null); };
  const handleTotpVerify = async () => { const response = await verifyTotp.mutateAsync(totpSetupCode); setBackupCodes(response.backup_codes || []); setTotpSetupCode(""); setTotpSecret(null); setTotpUri(null); setTotpFlow("idle"); setMfaNotice("Authenticator app enabled. Download and store your backup codes."); };
  const handleTotpDisable = async () => { await disableTotp.mutateAsync(totpDisableCode); setTotpDisableCode(""); setTotpFlow("idle"); setTotpSecret(null); setTotpUri(null); setMfaNotice("Multi-factor authentication disabled."); };
  const handleBackupCodes = async () => { if (!mfaEnabled) { setMfaNotice("Enable MFA first. Backup codes are only used with MFA."); return; } setMfaNotice("Generating backup codes. This can take a few seconds..."); const response = await regenerateBackupCodes.mutateAsync(); setBackupCodes(response.backup_codes || []); setMfaNotice("New backup codes generated. Download and store them now."); };
  const handleBackupCodesDownload = React.useCallback(() => { if (typeof window === "undefined" || backupCodes.length === 0) return; const blob = new Blob([buildBackupCodesText(backupCodes)], { type: "text/plain;charset=utf-8" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `bunoraa-backup-codes-${new Date().toISOString().slice(0, 10)}.txt`; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url); }, [backupCodes]);

  const copyMfaValue = React.useCallback(async (value: string, label: string) => {
    if (typeof navigator === "undefined" || !navigator.clipboard) { setMfaNotice(`Unable to copy ${label.toLowerCase()} on this device.`); return; }
    try { await navigator.clipboard.writeText(value); setMfaNotice(`${label} copied.`); } catch { setMfaNotice(`Unable to copy ${label.toLowerCase()} on this device.`); }
  }, []);

  React.useEffect(() => { if (!mfaEnabled) return; if (totpFlow !== "setup") return; setTotpFlow("idle"); setTotpSecret(null); setTotpUri(null); }, [mfaEnabled, totpFlow]);
  React.useEffect(() => { if (!mfaNotice) return; const timer = window.setTimeout(() => setMfaNotice(null), 5000); return () => window.clearTimeout(timer); }, [mfaNotice]);
  React.useEffect(() => { let active = true; if (!totpUri) { setTotpQrDataUrl(null); return () => { active = false; }; } try { const qr = qrcodeGenerator(0, "M"); qr.addData(totpUri); qr.make(); const svg = qr.createSvgTag(5, 1); const dataUrl = `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`; if (active) setTotpQrDataUrl(dataUrl); } catch { if (active) setTotpQrDataUrl(null); } return () => { active = false; }; }, [totpUri]);
  ...
  const handleToggleDisableForm = () => { setTotpDisableCode(""); setTotpFlow((prev) => (prev === "disable" ? "idle" : "disable")); };
  const handleRegisterPasskey = async () => {
    if (typeof window === "undefined" || !window.PublicKeyCredential) return;
    const options = await registerPasskeyOptions.mutateAsync();
    const decoded = decodeCreationOptions(options as unknown as PublicKeyCredentialCreationOptions);
    const credential = (await navigator.credentials.create({ publicKey: decoded })) as PublicKeyCredential | null;
    if (!credential) return;
    await registerPasskeyVerify.mutateAsync({ credential: encodeCredential(credential), nickname: passkeyName });
    setPasskeyName("");
  };
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card variant="bordered" className="space-y-4">
          ...
          <form className="space-y-3" onSubmit={form.handleSubmit(handlePasswordChange)}>
            ...
            <label className="block text-sm">Confirm new password<input className="mt-2 w-full rounded-lg border border-border bg-card px-3 py-2" type="password" autoComplete="new-password" {...form.register("new_password_confirm")} /></label>
        ...
        <Card variant="bordered" className="space-y-4">
          ...
          {showTotpSetupForm ? (
            <div className="space-y-2 rounded-lg border border-border bg-muted p-3 text-sm">
              ...
              <label className="block text-sm">Step 3: Enter 6-digit code from app<input className="mt-2 w-full rounded-lg border border-border bg-card px-3 py-2" value={totpSetupCode} onChange={(event) => setTotpSetupCode(event.target.value)} inputMode="numeric" autoComplete="one-time-code" placeholder="123456" /></label>
              <div className="grid gap-2 sm:flex sm:flex-wrap"><Button className="w-full sm:w-auto" onClick={handleTotpVerify} disabled={verifyTotp.isPending || totpSetupCode.trim().length < 6}>{verifyTotp.isPending ? "Enabling..." : "Enable MFA"}</Button><Button variant="ghost" className="w-full sm:w-auto" onClick={() => { setTotpFlow("idle"); setTotpSecret(null); setTotpUri(null); setTotpSetupCode(""); }}>Cancel</Button></div>
            </div>
          ) : null}
          {showTotpDisableForm ? (
            <div className="space-y-2 rounded-lg border border-border bg-muted p-3 text-sm">
              <p className="font-semibold">Confirm with your authenticator code to disable MFA</p>
              <label className="block text-sm">Verification code<input className="mt-2 w-full rounded-lg border border-border bg-card px-3 py-2" value={totpDisableCode} onChange={(event) => setTotpDisableCode(event.target.value)} inputMode="numeric" autoComplete="one-time-code" placeholder="123456" /></label>
              <div className="grid gap-2 sm:flex sm:flex-wrap"><Button variant="ghost" className="w-full sm:w-auto" onClick={handleTotpDisable} disabled={disableTotp.isPending || totpDisableCode.trim().length < 6}>{disableTotp.isPending ? "Disabling..." : "Disable MFA"}</Button><Button variant="ghost" className="w-full sm:w-auto" onClick={() => setTotpFlow("idle")}>Cancel</Button></div>
            </div>
          ) : null}
          {backupCodes.length ? (
