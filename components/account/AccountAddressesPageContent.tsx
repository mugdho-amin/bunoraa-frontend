...
export function AccountAddressesPageContent() {
        ...
        <div className="grid gap-4 md:grid-cols-2">{addressesQuery.data.map((address) => (
          <Card key={address.id} variant="bordered" className="space-y-2 p-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button asChild size="sm" variant="ghost"><Link href={`/account/addresses/${address.id}/set-default/`}>Set default</Link></Button>
            </div>
          </Card>
        ))}</div>
