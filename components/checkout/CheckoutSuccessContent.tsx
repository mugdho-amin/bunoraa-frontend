      ...
      <div className="min-h-screen bg-background text-foreground">
        <div className="mx-auto w-full max-w-4xl px-3 sm:px-5 py-16">
          <Card variant="bordered" className="space-y-6">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-foreground/60">{t("order_confirmed", "Order confirmed")}</p>
              <h1 className="text-3xl font-semibold">{t("thank_you", "Thank you for your purchase")}</h1>
              <p className="mt-2 text-sm text-foreground/60">{t("processing_order_now", "We're processing your order now.")}</p>
            </div>
            {orderQuery.isLoading ? <p className="text-sm text-foreground/60">{t("loading_order_details", "Loading order details...")}</p>
            : orderQuery.isError ? (
              <div className="space-y-2">
                <p className="text-sm text-foreground/60">{t("order_details_unavailable", "We couldn't load full order details yet.")}</p>
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="rounded-xl border border-border bg-card p-4">
                  <p className="text-xs uppercase tracking-[0.2em] text-foreground/60">{t("shipping_to", "Shipping to")}</p>
                  ...
                  <p className="text-foreground/70">{formatAddressLine(orderQuery.data.shipping_address)}</p>
                </div>
              </div>
            ) : orderNumber ? (
              <div className="space-y-2 text-sm">
                <button type="button" className="text-left text-sm text-foreground/60 underline underline-offset-4 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40" onClick={() => handleCopyOrderNumber(orderNumber)} title="Copy order number">Order #{orderNumber}</button>
                <p className="text-sm text-foreground/60">{t("email_updates", "Your order is confirmed. We'll email you with updates.")}</p>
              </div>
            ) : <p className="text-sm text-foreground/60">{t("order_confirmed_simple", "Your order is confirmed.")}</p>}
