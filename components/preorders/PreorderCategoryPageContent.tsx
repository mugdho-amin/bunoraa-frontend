...
export function PreorderCategoryPageContent({ category }: { category: PreorderCategory }) {
  return (
      ...
      <div className="grid gap-4 sm:gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <Card variant="bordered" className="space-y-4 p-4 sm:p-5">
          <div className="flex items-start justify-between gap-4">
            ...
            {category.image ? <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-2xl bg-muted sm:h-20 sm:w-20"><Image src={category.image} alt={category.name} fill sizes="80px" loading="lazy" decoding="async" className="object-cover" /></div> : null}
          </div>
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:gap-3">
            <Button asChild variant="primary-gradient" className="w-full sm:w-auto"><Link href={`/preorders/create/1/?category=${category.slug}`}>Start preorder</Link></Button>
            <Button asChild variant="secondary" className="w-full sm:w-auto"><Link href="/preorders/">Back to categories</Link></Button>
          </div>
        </Card>
        <Card variant="glass" className="space-y-3 p-4 sm:p-5">
          ...
          <ul className="space-y-2 text-sm text-foreground/70">
            ...
            <li>Approval required: {category.requires_approval ? "Yes" : "No"}</li>
            {category.allow_rush_order ? <li>Rush fee: {category.rush_order_fee_percentage}%</li> : null}
          </ul>
        </Card>
      </div>
