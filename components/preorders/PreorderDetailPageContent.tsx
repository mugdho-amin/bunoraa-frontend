...
export function PreorderDetailPageContent({ preorderNumber }: { preorderNumber?: string }) {
  const { push } = useToast();
  ...
  const statusHistory = preorder?.status_history ? [...preorder.status_history].reverse() : [];

  const handleSendMessage = async () => {
    if (!messageBody.trim()) { push("Please write a message.", "error"); return; }
    try {
      await actions.sendMessage.mutateAsync({ message: messageBody.trim(), subject: messageSubject.trim() });
      setMessageBody(""); setMessageSubject(""); push("Message sent.", "success");
  ...
  const handleUpload = async (type: "design" | "reference") => {
    ...
    try {
      if (type === "design") {
        ...
        setDesignFiles([]); push("Design files uploaded.", "success");
      } else {
        await Promise.all(files.map((file) => actions.uploadReference.mutateAsync({ file })));
        setReferenceFiles([]); push("Reference files uploaded.", "success");
      }
    } catch (error) { push(getErrorMessage(error), "error"); }
  };
  ...
  const handleAcceptQuote = async (quote: PreorderQuote) => {
    if (!quote.id) return;
    if (!window.confirm("Accept this quote and proceed to deposit?")) return;
    try { await actions.acceptQuote.mutateAsync(quote.id); push("Quote accepted.", "success"); }
    catch (error) { push(getErrorMessage(error), "error"); }
  };
  ...
  const handleRejectQuote = async (quote: PreorderQuote) => {
    ...
    try { await actions.rejectQuote.mutateAsync({ quoteId: quote.id, reason: rejectReasons[quote.id] || "" }); push("Quote rejected.", "success"); }
    catch (error) { push(getErrorMessage(error), "error"); }
  };
              ...
              <div className="space-y-4 sm:space-y-6">
                ...
                <Card variant="bordered" className="space-y-4 p-4 sm:p-5">
                  ...
                  <div className="grid gap-3">
                    ...
                    <textarea className="min-h-[120px] w-full rounded-lg border border-border bg-card px-3 py-2 text-sm" placeholder="Write your message" enterKeyHint="send" value={messageBody} onChange={(event) => setMessageBody(event.target.value)} />
                    <div><Button className="w-full sm:w-auto" variant="primary-gradient" onClick={handleSendMessage} disabled={actions.sendMessage.isPending}>{actions.sendMessage.isPending ? "Sending..." : "Send message"}</Button></div>
                  </div>
                </Card>
              </div>
              <div className="space-y-4 sm:space-y-6 lg:sticky lg:top-24 lg:self-start">
                <Card variant="bordered" className="space-y-3 p-4 sm:p-5">
                  ...
                  <div className="space-y-2 text-sm text-foreground/70">
                    ...
                    <p>Paid to date: {formatMoney(preorder.amount_paid || 0, currency)}</p>
                    <p className="font-semibold">Remaining: {formatMoney(preorder.amount_remaining || 0, currency)}</p>
                  </div>
                </Card>
                ...
                <Card variant="bordered" className="space-y-3 p-4 sm:p-5">
                  ...
                  <div className="text-sm text-foreground/70">
                    ...
                    {preorder.tracking_url ? <a href={preorder.tracking_url} target="_blank" rel="noreferrer" className="mt-1 inline-block break-all text-primary">Track shipment</a> : null}
                  </div>
                </Card>
                          ...
                          {quote.status === "pending" || quote.status === "sent" ? (
                            <div className="mt-3 space-y-2">
                              ...
                              <div className="grid gap-2 sm:flex sm:flex-wrap">
                                <Button className="w-full sm:w-auto" size="sm" variant="primary-gradient" onClick={() => handleAcceptQuote(quote)} disabled={actions.acceptQuote.isPending}>Accept quote</Button>
                                <Button className="w-full sm:w-auto" size="sm" variant="secondary" onClick={() => handleRejectQuote(quote)} disabled={actions.rejectQuote.isPending}>Reject quote</Button>
                              </div>
                            </div>
                          ) : null}
                        </div>
