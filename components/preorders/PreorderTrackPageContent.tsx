...
const getErrorMessage = (error: unknown) => error instanceof Error ? error.message : "Unable to track preorder.";

export function PreorderTrackPageContent() {
  ...
  React.useEffect(() => {
    ...
    if (number && !form.preorder_number) setForm((prev) => ({ ...prev, preorder_number: number }));
  }, [searchParams, form.preorder_number]);

  const handleSubmit = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!form.preorder_number.trim() || !form.email.trim()) { push("Enter your preorder number and email.", "error"); return; }
    try { const data = await tracking.mutateAsync({ preorder_number: form.preorder_number.trim(), email: form.email.trim() }); setResult(data); }
    catch (error) { push(getErrorMessage(error), "error"); setResult(null); }
  };
      ...
      <div className="grid gap-4 sm:gap-6 lg:grid-cols-[1fr_1.2fr]">
        <Card variant="bordered" className="space-y-4 p-4 sm:p-5">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <Button type="submit" className="w-full" disabled={tracking.isPending}>{tracking.isPending ? "Checking..." : "Track preorder"}</Button>
          </form>
        </Card>
        <Card variant="bordered" className="space-y-4 p-4 sm:p-5">
