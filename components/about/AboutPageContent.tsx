...
const getAboutPage = cache(async (): Promise<PageDetail | null> => { try { const response = await apiFetch<PageDetail>("/pages/about/"); return response.data; } catch (error) { if (error instanceof ApiError && error.status === 404) return null; return null; } });
